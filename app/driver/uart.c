...
uart0_rx_intr_handler(void *para)
{
   ...
   // uint8 RcvChar;
    ...
    if(UART_FRM_ERR_INT_ST == (READ_PERI_REG(UART_INT_ST(UART0)) & UART_FRM_ERR_INT_ST))//if tran error
    {
    	uart_sendString("FRM_ERR\r\n");
        WRITE_PERI_REG(UART_INT_CLR(UART0), UART_FRM_ERR_INT_CLR);
    }

    if(UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(UART0)) & UART_RXFIFO_FULL_INT_ST))//if Rxfifo is full
    {
      uart_sendString("fifo full\r\n");
      ETS_UART_INTR_DISABLE();/////////

      system_os_post(my_recvTaskPrio, 0, 0);
     }
    else if(UART_RXFIFO_TOUT_INT_ST == (READ_PERI_REG(UART_INT_ST(UART0)) & UART_RXFIFO_TOUT_INT_ST))//if Rxfifo is tout
    {
       ETS_UART_INTR_DISABLE();
       system_os_post(my_recvTaskPrio, 0, 0);
     }
